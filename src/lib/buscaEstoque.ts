...
// aparecer em algum lugar (código, descrição, categoria ou lote) — não
// precisa ser um trecho contínuo nem a palavra inteira. "PARAF M16 X 50"
// acha "PARAFUSO SEXTAVADO M16 X 50 DIN...", mesmo abreviado e fora de ordem.
export function filtrarItensEstoque(itens: EstoqueItem[], busca: string): EstoqueItem[] {
  ...
  return itens.filter((item) => {
    const alvo = `${item.codigo} ${item.descricao} ${item.categoria ?? ''} ${item.lotes ?? ''}`.toLowerCase()
